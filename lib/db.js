...
async function record (
    url, dwellTime, filterListUrl, filterListText, records, depth, breath,
    tags, parentCrawlId = null, debug = false
) {
    DEBUG_MESSAGE = msg => {
        if (debug === true) {
            console.log("db: " + msg);
    ...
    const crawlId = await idForCrawl(client, siteId, dwellTime, url, depth, breath, parentCrawlId);
    if (Array.isArray(tags) && tags.length > 0) {
        await assignTagsToCrawlId(client, crawlId, tags);
    }
...
async function assignTagsToCrawlId (client, crawlId, tags) {
    const insertParams = [];
    const insertTerms = [];
    ...
    const joinQueryValues = tagIds.map(tagId => {
        return `(${crawlId}, ${tagId})`;
    });
    const insertJoinQuery = "INSERT INTO crawls_tags(crawl_id, tag_id) VALUES " + joinQueryValues.join(",") + " ON CONFLICT (crawl_id, tag_id) DO NOTHING;";
    await client.query(insertJoinQuery, []);
    return tagIds;
}
...
async function idForSite (client, url) {
    ...
    if (selectRs.rows && selectRs.rows.length === 1) {
        ...
        DEBUG_MESSAGE(`Found ${url} has sites.id = ${siteId}.`);
        return siteId;
    }
    ...
    DEBUG_MESSAGE(`Inserted ${url} into sites with id = ${siteId}.`);
    return siteId;
}


async function idForCrawl (client, siteId, dwellTime, url, depth, breath, parentCrawlId = null) {
    const insertQuery = `
        INSERT INTO
            crawls(site_id, dwell_time, url, depth, breath, parent_crawl_id)
        VALUES
            ($1, $2, $3, $4, $5, $6)
        RETURNING
            id;`;
    const insertArgs = [siteId, dwellTime, url, depth, breath, parentCrawlId];
...
async function recordAllowed (client, crawlId, allowedEntries) {
    ...
    const insertQuery = `
        INSERT INTO
            allowed_requests(crawl_id, url, base_url, initiator_url, resource_type_id, initiator_type_id, request_id, hash, timestamp)
        VALUES
            ${insertTerms.join(",")};
    `;
...
async function recordBlocks (client, crawlId, ruleToIdMap, logEntries) {
    ...
    const insertQuery = `
        INSERT INTO
            blocked_requests(rule_id, exception_rule_id, crawl_id, url, base_url, resource_type_id, initiator_type_id, initiator_url, request_id, hash, timestamp)
        VALUES
            ${insertTerms.join(",")};
    `;
    await client.query(insertQuery, insertParams);
