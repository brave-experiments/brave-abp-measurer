...
async function recordUnavailabeDomain (batchUuid, domain, rank, tags) {
    const client = getClient();
    const [batchId, domainId] = await registerBatchAndDomain(client, batchUuid, domain, rank, tags);
...
async function record (
    batchUuid, domain, url, dwellTime, filterListUrl, filterListText, records, depth,
    breath, tags, parentCrawlId = null, rank = null, debug = false
) {
    DEBUG_MESSAGE = msg => {
        if (debug === true) {
            console.log("db: " + msg);
    ...
    const crawlId = await idForCrawl(
        client, batchId, domainId, dwellTime, url,
...
async function registerBatchAndDomain (client, batchUuid, domain, rank, tags) {
    await client.query("BEGIN");
    ...
    if (rank !== null) {
        await recordDomainAlexaRank(client, batchId, domainId, rank);
    }

    const domainId = await idForDomain(client, domain);

    await client.queryTerms("COMMIT");
...
async function idForBatch (client, batchUuid) {
    ...
    const selectRs = client.query(selectQuery, queryTerms);
...
async function recordDomainAlexaRank (client, batchId, domainOrDomainId, rank) {
    let domainId;
    if (typeof domainOrDomainId === "number") {
        domainId = domainOrDomainId;
    } else {
    ...
    const insertQuery = "INSERT INTO batches_domains_ranks(domain_id, batch_id, rank) VALUES ($1, $2, $3) RETURNING id;";
    const insertRs = await client.query(insertQuery, [domainId, batchId, rank]);
    const resultId = insertRs.rows[0].id;
...
async function idForCrawl (
    client, batchId, domainId, dwellTime, url, depth, breath,
    parentCrawlId = null
) {
    const insertQuery = `
        INSERT INTO
