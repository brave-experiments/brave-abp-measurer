...
const getClient = () => {
    ...
    client.connect();
    debugLib.verbose("Connected to database");
    return client;
};


async function recordUnavailableDomain (batchUuid, domain, rank, tags, region) {
...
async function idForBatch (client, batchUuid) {
    ...
    if (selectRs.rows && selectRs.rows.length === 1) {
        ...
        debugLib.verbose(`Found ${batchUuid} has batches.id = ${batchId}.`);
        return batchId;
    }
    ...
    try {
        ...
        debugLib.verbose(`Inserted ${batchUuid} into batches with id = ${batchId}.`);
        return batchId;
    } catch (error) {
        debugLib.verbose(`A batch with UUID ${batchUuid} was inserted underneath us.  Re-fetching.`);
...
async function idsForChunkOfRules (client, listId, rules) {
    ...
    debugLib.verbose("About to record " + rules.length + " filter rules for this list");
    let index = 0;
    rules.forEach(rule => {
...
async function assignTagsToBatch (client, batchId, tags) {
    ...
    let offset = 0;
    debugLib.verbose(`About to insert/fetch ids for tags: ${tags.join(", ")}.`);
    tags.forEach(tag => {
...
async function idsForRulesInFilterList (client, filterListUrl, filterListText) {
    ...
    if (selectRs.rows && selectRs.rows.length === 1) {
        ...
        debugLib.verbose(`Already seen list as id "${listId}", so returning already existing rule ids.`);
    ...
    debugLib.verbose(`Recorded "${filterListUrl}" as list id "${listId}"`);
...
async function recordDomainNotReachable (client, batchId, domainOrDomainId) {
    ...
    debugLib.verbose(`Inserted domains_unreachable record with id ${resultId}`);
    return resultId;
}
...
async function recordDomainAlexaRank (client, batchId, domainOrDomainId, rank, region) {
    ...
    if (selectRs.rows.length > 0) {
        ...
        debugLib.verbose(`Found existing batches_domains_ranks record (batch_id = ${batchId}, domain_id = ${domainId}) with id ${resultId}`);
    ...
    const resultId = insertRs.rows[0].id;
    debugLib.verbose(`Inserted batches_domains_ranks record with id ${resultId}`);
    return resultId;
}
...
async function idForDomain (client, domain) {
    ...
    if (selectRs.rows && selectRs.rows.length === 1) {
        ...
        debugLib.verbose(`Found ${domain} has domain.id = ${domainId}.`);
        return domainId;
    }
    ...
    debugLib.verbose(`Inserted ${domain} into domains with id = ${domainId}.`);
    return domainId;
}
...
async function idForCrawl (
    ...
    debugLib.verbose(`Inserted crawl for ${url} into crawls with id ${crawlId}.`);
...
async function recordUrlIfHttp (client, url, baseUrl = undefined) {
    ...
    const insertQuery = `
        ...
        DO UPDATE SET
            last_seen = now();
    `;
    const insertParams = [parsedUrl.hostname];
...
async function recordAllowed (client, crawlId, allowedEntries) {
    ...
    for (const record of allowedEntries) {
        const initiatorUrl = record.initiator.url || null;
        const insertRow = [];
        ...
        insertParams.push(initiatorUrl);
        insertRow.push("$" + (++index));
        ...
        await recordUrlIfHttp(record.for, initiatorUrl);

        if (record.from) {
            await recordUrlIfHttp(record.from, initiatorUrl);
        }
    }

    debugLib.verbose(`Inserting ${insertTerms.length} allow records.`);
    const insertQuery = `
        INSERT INTO
...
async function recordBlocks (client, crawlId, ruleToIdMap, logEntries) {
    ...
    for (const record of logEntries) {
        ...
        if (!ruleId) {
            debugLib.verbose("FIND-ME");
            debugLib.verbose(ruleToIdMap);
            debugLib.verbose(record);
            continue;
        }
        const initiatorUrl = record.entry.initiator.url || null;
        const insertRow = [];
        ...
        await recordUrlIfHttp(record.entry.for, initiatorUrl);

        if (record.entry.from) {
            await recordUrlIfHttp(record.entry.from, initiatorUrl);
        }
    }

    debugLib.verbose(`Inserting ${insertTerms.length} block records.`);
    const insertQuery = `
        INSERT INTO
