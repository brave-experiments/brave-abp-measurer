...
async function record (url, dwellTime, filterListUrl, filterListText, records, debug = false, prevCrawlId = null) {
    ...
    const crawlId = await idForCrawl(client, siteId, dwellTime, url, prevCrawlId);
...
async function idsForRulesInFilterList (client, filterListUrl, filterListText) {
    ...
    const ruleChunks = utils.chunkArray(abpRules, 2000);
...
async function idForCrawl (client, siteId, dwellTime, url, parentCrawlId = null) {
    const insertQuery = `
        INSERT INTO
            crawls(site_id, dwell_time, url, parent_crawl_id)
        VALUES
            ($1, $2, $3, $4)
        RETURNING
            id;`;
    const insertRs = await client.query(insertQuery, [siteId, dwellTime, url, parentCrawlId]);
    return insertRs.rows[0].id;
};
