...
async function crawlPromise (
    ...
    fetchChildLinks = false, debug = false
) {
    DEBUG_MESSAGE = msg => {
        if (debug === true) {
    ...
    try {
        ...
        DEBUG_MESSAGE(`Found ${collectedRequests.length} requests to fetch info for.`);
        for (const recordEntry of collectedRequests) {
            const recordRequestId = recordEntry.requestId;
            try {
                DEBUG_MESSAGE(`Requesting resource for id ${recordRequestId}`);
                ...
                DEBUG_MESSAGE(JSON.stringify(recordEntry));
                DEBUG_MESSAGE(e);
            }
        }
