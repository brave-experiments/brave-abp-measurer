...
const {Capabilities, By} = webdriver;
...
async function crawlPromise (url, filterListText, chromePath, chromeDriverPath, additionalArgs) {
    debugMessage = msg => {
    ...
    try {
        ...
        let hrefs;
        if (fetchAnchors === true) {
            debugMessage("Fetching anchor elements on the page");
            const anchorElements = await driver.findElements(By.css("a"));
            let anchorHrefs = new Set();
            for (let anchor of anchorElements) {
                anchorHrefs.add(await anchor.getAttribute("href"));
            }
            debugMessage(`Found ${anchorHrefs.length} unique a[href]'s.`);
